...
export const login = actionClient.inputSchema(schema).action(async ({ parsedInput }) => {
...
returnValidationErrors(schema, {
  password: {
    _errors: ["Must contain at least one uppercase letter", "Must contain at least one number"],
  },
});
```
