...
async function main() {
  ...
  const argLoaderStyle = parseArg("--loader-style") as
    | import("./config.js").LoaderConfig["style"]
    | undefined;
  ...
  async function runDemo() {
    ...
    emit({
      ...
      name: "list_dir",
      callId: "c2",
      output: "src/ package.json tsconfig.json .env",
    });
    ...
    emit({
      ...
      name: "list_dir",
      callId: "c2b",
      output: "cli.ts agent.ts config.ts tools/",
    });
    ...
    emit({
      ...
      name: "grep",
      callId: "c5",
      output: "src/agent.ts:export async function runAgent",
    });
