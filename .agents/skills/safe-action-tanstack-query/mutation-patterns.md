...
export function CreatePostButton() {
  ...
  const mutation = useMutation(
    mutationOptions(createPostAction, {
      onSuccess: (data) => {
...
function ErrorDisplay({
  ...
  error: ActionMutationError<string, Record<string, { _errors?: string[] }>>;
}) {
      ...
      {mutation.isError && (
        <>
          ...
          {hasValidationErrors(mutation.error) && (
            <p className="text-red-500">{mutation.error.validationErrors.email?._errors?.[0]}</p>
          )}
        </>
      )}
    </form>
...
const mutation = useMutation(
  mutationOptions(toggleTodoAction, {
