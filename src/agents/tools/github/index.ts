...
interface ReadGitHubFilesOptions {
  provider: GitHubProvider;
}

export const createReadGitHubFiles = ({ provider }: ReadGitHubFilesOptions) => {
  return tool({
    description:
    ...
    execute: async ({ files }) => provider.readFiles(files),
      ...
      files: z
        .array(
          z.object({
            owner: z
              .string()
              .describe("The repository owner (user or organization), e.g. 'vercel'."),
            path: z
              .string()
