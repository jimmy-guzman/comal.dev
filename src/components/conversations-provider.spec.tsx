...
const wrapperWith = (initial: RecentConversation[]) => {
  return ({ children }: { children: React.ReactNode }) => {
    return <ConversationsProvider initial={initial}>{children}</ConversationsProvider>;
  };
};

describe("ConversationsProvider", () => {
  it("should expose the initial list to consumers", () => {
    const { result } = renderHook(() => useConversations(), {
      wrapper: wrapperWith([conv("1"), conv("2")]),
    });
  ...
  it("should update an existing conversation title", () => {
    const { result } = renderHook(() => useConversations(), {
      wrapper: wrapperWith([conv("1", "old"), conv("2", "keep")]),
    });
