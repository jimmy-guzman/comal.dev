...
sequenceDiagram
  ...
  opt agent has memory-search attached
    ...
    API->>DB: cosine similarity search (top 5, >=0.4)
  ...
  API->>LLM: embedMany pending memories
  API->>DB: write embeddings
  API->>LLM: generate conversation title (first turn)
```

The assistant turn is persisted only after the response is sent. Budget spend is recorded from the turn's token usage. Memory lookup runs only when the agent has `memory-search` attached; memory-tool saves write rows with `embedding = NULL` and the `after()` batch fills them in, so a memory saved mid-turn is searchable on the next turn.
...
Tool ids are resolved against the static registry (`src/agents/tools/registry.ts`). Sub-agent edges become tools that recurse through `loadAgent` up to `MAX_DEPTH = 2`, so a child can have its own sub-agents (root, child, grandchild). The result is an `AgentConfig` the chat route hands to `streamText`. Memory tools (`memory-save`, `memory-search`, `memory-delete`) flow through the same picker, registry, and `buildToolsRecord` path as everything else; an agent has memory if (and only if) the relevant tool is attached.
...
erDiagram
  ...
  agent ||--o{ memory : "saved by (nullable)"
...
- **List index pages** (`/chats`, `/agents`, `/tools`, `/memories`) share the same list chrome (`h1`, header row with optional primary action, `max-w-5xl`) and use shadcn `Item` / `ItemGroup` in a responsive grid (`variant="outline"` tiles). `/tools` repeats that pattern once per tool registry group section.
...
- **Persistent user memory is just three tools.** Memory lives in two tables (`src/db/schemas/memory-schema.ts`): `memory` (one row per fact, `embedding vector(1536)` with an HNSW + `vector_cosine_ops` index) and `user_memory_settings` (per-user cap, default 500, CHECK-constrained to 1..10000). The three tools (`memory-save`, `memory-search`, `memory-delete` in `src/agents/tools/memory/`) attach through the same picker, registry, and `buildToolsRecord` path as every other tool; there is no parallel toggle. `memory-save` and `memory-delete` are writes and approval-gateable like other write tools. Memory is user-scoped, not agent-scoped, so any agent with the relevant tool attached reads the same pool; `memory.source_agent_id` records who saved each row for the UI badge but is not used for lookup. Embeddings reuse OpenRouter via `openrouter.textEmbeddingModel("openai/text-embedding-3-small")`; rows are inserted with `embedding = NULL` and embedded in the chat route's `after()` hook via `embedMany`, so a memory saved mid-conversation isn't searchable until the next turn. The chat route auto-injects matches when `memory-search` is attached: embed the latest user message, cosine-similarity search top 5 above 0.4, append as a `<memory>` block to the system prompt with each row HTML-entity-escaped to neutralize tag breakout. The threshold is calibrated for `text-embedding-3-small`, which produces lower cosine scores than the issue-quoted 0.75 — a literal "favorite coffee equipment brewing method" query against a "User's favorite coffee equipment brand is Fellow" memory hits 0.61, well below the original cutoff. 0.4 catches realistic recall queries while still filtering unrelated noise. This is a perf optimization on top of the search tool, saving the model a round trip. Cap enforcement happens inside a transaction that upserts the settings row and `SELECT ... FOR UPDATE`s it before counting and inserting, so concurrent saves can't both pass the check.
