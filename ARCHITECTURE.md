...
sequenceDiagram
  ...
  opt agent.enableMemory && user message present
...
The assistant turn is persisted only after the response is sent. Budget spend is recorded from the turn's token usage. Memory lookup runs only when the agent has `enable_memory` set; memory-tool saves write rows with `embedding = NULL` and the `after()` batch fills them in, so a memory saved mid-turn is searchable on the next turn.
...
Tool ids are resolved against the static registry (`src/agents/tools/registry.ts`). Sub-agent edges become tools that recurse through `loadAgent` up to `MAX_DEPTH = 2`, so a child can have its own sub-agents (root, child, grandchild). The result is an `AgentConfig` the chat route hands to `streamText`. Memory tools (`memory-save`, `memory-search`, `memory-delete`) are gated twice in `buildToolsRecord`: skipped when the agent's `enable_memory` is false, and skipped for sub-agents (`depth > 0`) regardless of the parent's flag, so sub-agents never get memory tools.
...
- **Persistent user memory is user-scoped and opt-in per agent.** Memory lives in two tables (`src/db/schemas/memory-schema.ts`): `memory` (one row per fact, `embedding vector(1536)` with an HNSW + `vector_cosine_ops` index) and `user_memory_settings` (per-user cap, default 500, CHECK-constrained to 1..10000). `agent.enable_memory` gates access: agents with the flag set get the three memory tools (`memory-save`, `memory-search`, `memory-delete` in `src/agents/tools/memory/`) and auto-injection; agents with the flag clear get neither, with memory tools attached to the agent's tool selection filtered out at load time in `buildToolsRecord` (`src/agents/index.ts`). Sub-agents never get memory tools regardless of the parent flag, filtered by the same `depth > 0` check. Memory is user-scoped, not agent-scoped, so every memory-enabled agent reads the same pool; `memory.source_agent_id` records who saved each row for the UI badge but is not used for lookup. Embeddings reuse OpenRouter via `openrouter.textEmbeddingModel("openai/text-embedding-3-small")`; rows are inserted with `embedding = NULL` and embedded in the chat route's `after()` hook via `embedMany`, so a memory saved mid-conversation isn't searchable until the next turn. Auto-injection runs before `streamText`: embed the latest user message, cosine-similarity search top 5 above 0.75, append as a `<memory>` block to the system prompt with each row HTML-entity-escaped to neutralize tag breakout. Cap enforcement happens inside a transaction that upserts the settings row and `SELECT ... FOR UPDATE`s it before counting and inserting, so concurrent saves can't both pass the check.
